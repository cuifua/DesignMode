...
public class DepUserMediatorImpl
{
	...
	public boolean deleteDep(String depId)
	{
		...
		Collection<DepUserModel> tempCol = new ArrayList<DepUserModel>();//设置一个临时的集合，记录需要清除的关系对象
		for(DepUserModel du : depUserCol)
		{
			if(du.getDepId().equals(depId))
				tempCol.add(du);      //2：需要把这个相关的记录去掉，先记录下来
		}

		depUserCol.removeAll(tempCol);//3：从关系集合里面清除掉这些关系
		return true;
	}
